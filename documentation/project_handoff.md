...
Current June 8 hardware state: the new `STARTUPE2.CFGMCLK` JTAG-clock bitstream and probes were built successfully at `cosmic_ray_detection.runs/impl_1/cosmic_top.bit` and `.ltx` with timestamps around 15:07 on 2026-06-08. The first retry against the earlier BD-clock build still showed `debug hub core was not detected`. After the CFGMCLK rebuild, the board disappeared from USB/JTAG before hardware verification: Windows listed only COM7/COM8 Bluetooth ports, not COM3, and Vivado reported `ERROR: No hardware targets reported by hw_server.` Hardware acceptance is blocked until the board/cable is physically re-enumerated; after reseating the cable, run `.\run_jtag_diagnostics.bat 5 FF 1 PROGRAM`.
